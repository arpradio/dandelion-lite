-- Cardano GraphQL API was widely used at beginnings of Cardano History but now project is unmaintained. 
-- Old projects can use this API to try to migrate from GraphQL into cardano-sql or postgrest
CREATE SCHEMA IF NOT EXISTS cardano_graphql ;

-- SQL scripts from official cardano-graphql repository 

CREATE OR REPLACE VIEW cardano_graphql."AdaPots" AS
  SELECT
    epoch_no AS "epochNo",
    deposits,
    fees,
...
-- To support again this table we should adapt it as a view based on the current cardano-db-sync data
-- CREATE TABLE IF NOT EXISTS cardano_graphql."Asset" (
...
CREATE OR REPLACE VIEW cardano_graphql."Block" AS
...
CREATE OR REPLACE VIEW cardano_graphql."Cardano" AS
...
CREATE OR REPLACE VIEW cardano_graphql."CollateralInput" AS
SELECT
...
CREATE OR REPLACE VIEW cardano_graphql."CollateralOutput" AS
SELECT
  address,
...
CREATE OR REPLACE VIEW cardano_graphql."Delegation" AS
SELECT
  delegation.id AS "id",
...
CREATE OR REPLACE VIEW cardano_graphql."Epoch" AS
SELECT
  epoch.fees AS "fees",
...
CREATE OR REPLACE VIEW cardano_graphql."Datum" AS
SELECT
  bytes,
  hash,
  id,
...
CREATE OR REPLACE VIEW cardano_graphql."RedeemerDatum" AS
SELECT
  bytes,
  hash,
  id,
...
CREATE OR REPLACE VIEW cardano_graphql."ProtocolParams" AS
SELECT
...
CREATE OR REPLACE VIEW cardano_graphql."Redeemer" AS
SELECT
  redeemer.fee AS "fee",
  redeemer.id AS "id",
...
CREATE OR REPLACE VIEW cardano_graphql."ReferenceInput" AS
SELECT
  source_tx_out.address,
  source_tx_out.value,
  tx.hash AS "txHash",
...
CREATE OR REPLACE VIEW cardano_graphql."Reward" AS
SELECT
  reward.amount,
  stake_address.view AS "address",
...
CREATE OR REPLACE VIEW cardano_graphql."Script" AS
SELECT
  script.hash AS "hash",
  script.id AS "id",
...
CREATE OR REPLACE VIEW cardano_graphql."SlotLeader" AS
SELECT
...
CREATE OR REPLACE VIEW cardano_graphql."StakeDeregistration" AS
SELECT
...
CREATE OR REPLACE VIEW cardano_graphql."StakePoolOwner" AS
SELECT
...
CREATE OR REPLACE VIEW cardano_graphql."StakePoolRetirement" AS
SELECT
...
CREATE OR REPLACE VIEW cardano_graphql."StakeRegistration" AS
SELECT
...
CREATE OR REPLACE VIEW cardano_graphql."ActiveStake" AS
SELECT
  stake_address.view AS "address",
  amount AS "amount",
  epoch_no as "epochNo",
...
CREATE OR REPLACE VIEW cardano_graphql."TokenMint" AS
SELECT
...
CREATE OR REPLACE VIEW cardano_graphql."TokenInOutput" AS
SELECT
...
CREATE OR REPLACE VIEW cardano_graphql."TransactionOutput" AS
SELECT
  address,
...
CREATE OR REPLACE VIEW cardano_graphql."Utxo" AS SELECT
  address,
  tx_out.address_has_script AS "addressHasScript",
  value,
  tx.hash AS "txHash",
...
CREATE OR REPLACE VIEW cardano_graphql."Withdrawal" AS
SELECT
...
-- Lets apply SELECT only permissions to these users for cardano_graphql
SELECT sql_private_extensions.grant_read_only_access('web_anon, authenticator', 'cardano_graphql');
