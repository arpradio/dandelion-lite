CREATE TABLE IF NOT EXISTS {{SCHEMA}}.asset_info_cache (
...
CREATE INDEX IF NOT EXISTS idx_first_mint_tx_id ON {{SCHEMA}}.asset_info_cache (first_mint_tx_id);
...
BEGIN
  ...
  IF (
    ...
    WHERE state = 'active'
      AND query ILIKE '%{{SCHEMA}}.asset_info_cache_update%'
  ...
  IF _asset_info_cache_last_tx_id IS NULL THEN
    ...
    TRUNCATE TABLE {{SCHEMA}}.asset_info_cache;
  ELSE
  ...
  INSERT INTO {{SCHEMA}}.asset_info_cache
    SELECT
      ma.id,
    ...
    FROM
      ...
      LEFT JOIN {{SCHEMA}}.asset_registry_cache AS arc ON arc.asset_policy = ENCODE(ma.policy,'hex') AND arc.asset_name = encode(ma.name,'hex')
    WHERE
      CASE WHEN _asset_info_cache_last_tx_id IS NOT NULL AND _asset_id_list IS NOT NULL
        THEN
          mtm.ident = ANY(_asset_id_list)
