...
# Job Files:
# cat /etc/cron.d/koios-lite/init_koios_lite_cron  >> "$cronFile" && echo >>"$cronFile"
cat /etc/cron.d/koios-artifacts/init_koios_artifacts_cron  >> "$cronFile" && echo >>"$cronFile"
cat /etc/cron.d/dbless-cardano-token-registry/init_dbless_cardano_token_registry_cron  >> "$cronFile" && echo >>"$cronFile"
