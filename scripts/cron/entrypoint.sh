...
# Job Files:
cat /etc/cron.d/dandelion-lite/init_cron  >> "$cronFile" && echo >>"$cronFile"
# cat /etc/cron.d/koios-lite/init_cron  >> "$cronFile" && echo >>"$cronFile"
# cat /etc/cron.d/koios-artifacts-1.3.2/init_cron  >> "$cronFile" && echo >>"$cronFile" # IMPORTANT: koios dbsync requirements are not compatible with cardano-graphql and blockfrost's ones
