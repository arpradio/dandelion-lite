...
next_epoch_no=$(psql ${DB_NAME} -h ${POSTGRES_HOST} -qbt -c "SELECT MAX(epoch_no) FROM epoch_stake_progress WHERE completed='t'" | tr -cd '[:alnum:]')

psql ${DB_NAME} -h ${POSTGRES_HOST} -qbt -c "SELECT ${KOIOS_ARTIFACTS_SCHEMA}.active_stake_cache_update(${next_epoch_no});" 1>/dev/null
