...
[[ ${current_slot_in_epoch} -ge ${min_slot} ]] &&  
  ...
  psql ${DB_NAME} -h ${POSTGRES_HOST} -c "INSERT INTO ${KOIOS_ARTIFACTS_SCHEMA}.epoch_info_cache (epoch_no, p_nonce) VALUES (${next_epoch}, '${next_epoch_nonce}') ON CONFLICT DO NOTHING;"
