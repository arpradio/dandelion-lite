...
RUN_MODE="${1:-run}"
NETWORK="${2:-mainnet}"
TESTING_URL="${3:-'https://127.0.0.1:3080'}"

# Example: ./tests.sh run mainnet 'https://api.mainnet.yaci.xyz/api/v1' 2>&1 | tee mainnet.log
# Example: ./tests.sh run preprod 'https://127.0.0.1:${HAPROXY_PORT}/blockfrost/api/v1' 2>&1 | tee mainnet.log
...
source "./${NETWORK}.sh" # OFFICIAL_URL, TEST_CASES and default headers overrides defined here
...
run_test() {
  ...
  cmd="${cmd//%%DEFAULT_GET_ARGS%%/$DEFAULT_GET_ARGS}"
  cmd="${cmd//%%DEFAULT_POST_ARGS%%/$DEFAULT_POST_ARGS}"
...
run_tests() {
  ...
  for entry in "${TEST_CASES[@]}"; do
    ...
    if [[ "$RUN_MODE" == "run" ]]; then
      echo "CMD for Received: $(run_test showCmd "$tmpl" "$TESTING_URL")"
      echo "CMD for Expected: $(run_test showCmd "$tmpl" "$OFFICIAL_URL")"
    fi
    ...
    # local received
    # received=$(run_test run "$tmpl" "$TESTING_URL" | jq -S .)
    # saveFile "$received_file" "$received"
    received_raw=$(run_test run "$tmpl" "$TESTING_URL")
    ...
    # local expected
    # expected=$(run_test run "$tmpl" "$OFFICIAL_URL" | jq -S .)
    # saveFile "$expected_file" "$expected"

    expected_raw=$(run_test run "$tmpl" "$OFFICIAL_URL")
